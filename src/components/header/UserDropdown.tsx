...
export default function UserDropdown() {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11 flex items-center justify-center bg-brand-500 text-white font-bold">
          {/* Tampilkan inisial huruf pertama dari nama jika foto tidak ada */}
          {user?.name ? user.name.charAt(0).toUpperCase() : "U"}
        </span>

        <span className="block mr-1 font-medium text-theme-sm">
          {/* Tampilkan nama asli user, fallback ke "User" jika kosong */}
          {user?.name || "User"}
        </span>
        <svg
      ...
      >
        <div>
          ...
          {/* Tambahan: Tampilkan Role/Jabatan User */}
          <span className="mt-1 inline-block rounded bg-brand-50 px-2 py-0.5 text-xs font-medium text-brand-600 dark:bg-brand-900/30 dark:text-brand-400">
        ...
        {/* Tombol Logout yang memanggil fungsi handleLogout */}
        <button
